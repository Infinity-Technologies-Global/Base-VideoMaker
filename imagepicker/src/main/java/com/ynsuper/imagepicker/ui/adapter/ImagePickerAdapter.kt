...
package com.ynsuper.imagepicker.ui.adapter
...
import com.ynsuper.imagepicker.ui.imagepicker.GlideLoader
...
class ImagePickerAdapter(context: Context, private val config: Config, private val imageSelectListener: OnImageSelectListener) : BaseRecyclerViewAdapter<ImagePickerAdapter.ImageViewHolder?>(context) {
    ...
    private val selectedImages = arrayListOf<ImageModel>()
    ...
    private fun selectOrRemoveImage(imageModel: ImageModel, position: Int) {
        if (config.isMultipleMode) {
            val selectedIndex = ImageHelper.findImageIndex(imageModel, selectedImages)
            if (selectedIndex != -1) {
                selectedImages.removeAt(selectedIndex)
                notifyItemChanged(position, ImageUnselected())
                val indexes = ImageHelper.findImageIndexes(selectedImages, imageModels)
            ...
            imageSelectListener.onSingleModeImageSelected(imageModel)
        }
    }
    ...
    fun setSelectedImages(selectedImageModels: ArrayList<ImageModel>) {
        this.selectedImages.clear()
        this.selectedImages.addAll(selectedImageModels)
        notifyDataSetChanged()
